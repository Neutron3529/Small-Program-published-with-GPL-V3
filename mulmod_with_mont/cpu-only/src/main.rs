    ...
    fn check_with_parallel_sieve(limit: u64) {
        ...
        blocks.into_iter().par_bridge().for_each(|(low, high)| {
            ...
            while let Some(i) = iter.next() {
                ...
                mont.mont_to_num(pow(mont.num_to_mont(s52::Base::THREE.0), mont))
                    .iter()
                    .zip(mont.base.0.iter())
                    .for_each(|(&x, &y)| {
                        if x == y - REM {
            ...
            let _ = iter.into_remainder().map(|x| {
                x.for_each(|p| {
...
fn check_table_with_parallel(mut start: u64, mut end: u64) {
    ...
    fn calc_idx(plus: u64) -> [u64; REMAIN_SIZE as usize] {
        ...
        assert_eq!(
            prime.iter().enumerate().fold(0, |s, (x, &b)| if b {
                res[s as usize] = plus + x as u64;
                s + 1
            } else {
                s
            }),
    ...
    assert!(
        REMAIN_SIZE % 8 == 0,
        "REMAIN_SIZE (={REMAIN_SIZE}) 应该是8的倍数"
    );
    (0..(end - start)).par_bridge().for_each(|blk_idx| {
        ...
        println!(
            "{}",
            format!(
                "  Done block {blk_idx} {:5.2}% (from {} to {}), found {coef:?}",
                blk_idx as f64 / (end - start) as f64 * 100.,
                start_idx + blk_idx * BLOCK_SIZE,
                start_idx + (blk_idx + 1) * BLOCK_SIZE
            )
        )
...
fn main() {
    std::thread::Builder::new()
        ...
        .spawn(|| {
            ...
            check_table_with_parallel(
                std::env::args()
                    .nth(2)
                    .map_or(None, |x| x.replace("_", "").parse().ok())
                    .unwrap_or(3),
                std::env::args()
                    .nth(1)
                    .map_or(None, |x| x.replace("_", "").parse().ok())
